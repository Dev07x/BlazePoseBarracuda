...
public class PoseVisuallizer3D : MonoBehaviour
{
    ...
    public Vector3[] GetLandmarks3D()
    {
        Vector3[] landmarks = new Vector3[detecter.vertexCount];
        for (int i = 0; i < detecter.vertexCount; i++)
        {
            var landmark = detecter.GetPoseWorldLandmark(i);
            landmarks[i] = new Vector3(landmark.x, landmark.y, landmark.z);
        }
        return landmarks;
    }
}
